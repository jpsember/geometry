...
public class Font {
	...
	private void generateBitmap(int fontSize) {
		...
		if (false) {
			warning("generating larger, bolder font");
			fontSize *= 2;
	...
	private void generateSprites() {
		mSpriteContext = new SpriteContext(
				OurGLRenderer.TRANSFORM_NAME_DEVICE_TO_NDC, true);
		...
		for (int i = 0; i < PRINTABLE_TOTAL; i++) {
			Rect bounds = new Rect(mCursor.x, mCursor.y, mLetterSize.x,
					mLetterSize.y);
			GLSpriteProgram s = new GLSpriteProgram(mSpriteContext, mTexture,
					bounds);
			mSprites[i] = s;
			advanceCursor();
		}
	...
	/**
	 * Set color of text generated by subsequent render() calls
	 */
