...
public class ConcreteStepper extends AlgorithmStepper {
  ...
  /**
   * This debug-only flag, if true, performs additional tests to verify that the
   * UI and OpenGL threads are cooperating.
   */
   ...
   * 
   * @param availableRect
   *          rectangle available, in device space (aspect ratio is significant,
   *          not the actual size)
   */
  ...
  /**
   * Set visible rectangle, in algorithm space; this is probably bigger than the
   * algorithm rect, which was constructed to fit within it
   * 
   * @param visibleRect
   */
  ...
  /**
   * Add a prefix to a background layer key that is unique to the current active
   * stack depth
   */
  ...
  /**
   * Set contents of stepper view to either the stepper controls, or some other
   * view
   * 
   * @param auxView
  ...
  void calculateAlgorithmTotalSteps() {
    ...
    synchronized (getLock()) {
      ...
      boolean atMax = (mOptions.readTargetStep() == mOptions.readTotalSteps());
  ...
  private void performAlgorithm() {
    try {
      ...
      try {
        ...
        mTotalSteps = mCurrentStep;
        mTargetStep = mCurrentStep;
        show(constructExceptionString(t, "ConcreteStepper.performAlgorithm"));
      }
    } catch (DesiredStepReachedException e) {
   ...
   *          subsequent entries
   */
  private String constructExceptionString(Throwable throwable, String haltPrefix) {
    ...
    AbstractWidget.Listener listener = new AbstractWidget.Listener() {
      ...
      public void valueChanged(AbstractWidget widget) {
        ...
        for (int j = 0; j < 2; j++) {
          ...
          if (id == ids[j + 2]) {
            int seekStep = mOptions.readTargetStep() + (j == 0 ? -1 : 1);
            seekStep = MyMath.clamp(seekStep, 0, mOptions.readTotalSteps());
            mOptions.setTargetStep(seekStep);
          }
        }
      }
  ...
  public void acquireLock() {
    if (VERIFY_LOCK) {
      if (aLockCounter != 0 && Thread.currentThread() != aLockActiveThread) {
        lockError();
      }
      aLockCounter++;
  ...
  private void lockError() {
    if (VERIFY_LOCK) {
      die("unexpected lock thread " + nameOf(Thread.currentThread()) + " vs "
          + nameOf(aLockActiveThread) + "; acquired:\n" + aLockAquireInfo);
    }
  }
  ...
  /**
   * Set rendering state. If false, any render operations will generate display
   * elements for later rendering; if true, render operations actually perform
   * the rendering.
   * 
