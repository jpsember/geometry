...
class TwinViewContainer {
   ...
   * @param context
   * @param mainView
   *          the main view (i.e. the original 'content' view), it will actually
   *          be placed within a SlidingPaneLayout, which becomes the new
   *          content view
   */
  ...
  LinearLayout getAuxilliaryView() {
  ...
  private void buildAuxilliaryView() {
    LinearLayout view = linearLayout(getContext(), true);
  ...
  private void addChildViews(View mainView) {
    ...
    final float PREFERRED_MAIN_WIDTH_INCHES = 1.6f;
  ...
  private LinearLayout mAuxilliaryView;
