...
public class Polygon implements Iterable<Point>, Renderable {
  ...
  public static Polygon testPolygon(int index) {
    ...
    if (index >= TESTPOLY_DRAGON_X && index < TESTPOLY_DRAGON_X + 15) {
    ...
    } else if (index == TESTPOLY_SPIROGRAPH || index == TESTPOLY_SPIROGRAPH2) {
      ...
      for (int i = 0; i < 250 / sf; i++) {
        ...
        Point pt = new Point(MyMath.sin(i / f2) * scale, MyMath.cos(i / f3)
            * scale);
    ...
    int pad = 80;
    poly.transformToFitRect(new Rect(pad, pad, 1400 - pad * 2, 1000 - pad * 2),
        true);
    return poly;
  }
  ...
  public static Polygon discPolygon(Point origin, float radius,
    ...
    for (int i = 0; i < numberOfSides; i++) {
      Point v = MyMath.pointOnCircle(new Point(300, 300), MyMath.M_DEG * i
          * (360.0f / numberOfSides), 100);
      polygon.add(v);
    }
    return polygon;
  }
   ...
   * 
   * @param bounds
   *          bounding rectangle; the polygon will fit in a disc centered within
   *          this rectangle
   * @param radii
   *          distance of vertices from center, where 0 is at the center, and 1
   *          is on the disc boundary
   */
  ...
  public static Polygon circleWithOrigin(Point origin, float radius,
    ...
    for (int i = 0; i < numVertices; i++) {
      Point pt = MyMath.pointOnCircle(origin,
          (i * MyMath.PI * 2) / numVertices, radius);
      p.add(pt);
    }
    return p;
  }
