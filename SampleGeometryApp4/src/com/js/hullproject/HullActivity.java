...
public class HullActivity extends GeometryStepperActivity implements Algorithm {
  ...
  private void prepareInput(AlgorithmInput input) {
    ...
    if (mOptions.getBooleanValue(USE_EDITOR_DISCS)) {
      ...
      for (int i = mOptions.getIntValue(NUMBER_OF_DISCS); i >= 0; i--) {
        Point origin = MyMath.randomPointInDisc(r, mAlgBounds.midPoint(),
            mAlgBounds.maxDim() * .4f);
        mDiscs.add(new Disc(origin, (float) Math.pow(r.nextFloat(), pow)
            * mAlgBounds.minDim() * .4f + mAlgBounds.minDim() * .01f));
      }
    }
  }
  ...
  public void run(AlgorithmStepper stepper, AlgorithmInput input) {
    ...
    for (int pass = 0; pass < 2; pass++) {
      ...
      if (mLowerHullFlag && !mOptions.getBooleanValue(INCLUDE_LOWER_HULL))
        continue;
      if (s.bigStep())
        s.show("Constructing " + (mLowerHullFlag ? "lower" : "upper") + " hull");
      ...
      for (int i = mDiscs.size() - 1; i >= 0; i--) {
        ...
        s.addLayer(BGND_ELEMENT_CURRENTDISC,
            s.highlighted(mRendering_CurrentDisc));
        processDisc(d);
  ...
  private void processDisc(Disc xDisc) {
    ...
    for (i = 0; i < mHullDiscs.size(); i++) {
      ...
      if (!isHullAngle(ux.angle())) {
        if (s.step())
          s.show("not a hull bitangent candidate", s.highlighted(uDisc),
              s.highlighted(ux));
        return;
      }
    ...
    for (j = mHullDiscs.size() - 1; j >= 0; j--) {
      ...
      if (!isHullAngle(xv.angle())) {
        if (s.step())
          s.show("not a hull bitangent candidate", s.highlighted(vDisc),
              highlight(xv));
        return;
      }
    ...
    for (int k = 0; k < remCount; k++) {
      if (s.step())
        s.show("Removing hull disc ", s.highlighted(mHullDiscs.get(i + 1)));
  ...
  private Renderable highlight(Bitangent bitangent) {
    return s
        .highlighted(Segment.directed(bitangent.maPoint, bitangent.mbPoint));
  }
